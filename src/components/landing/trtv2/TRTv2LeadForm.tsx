...
export const TRTv2LeadForm = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.dispatchEvent(
      new CustomEvent("lp_trt_v2_form_submit", {
        detail: { name, phone, location, source: "lp-trt-v2", service: "trt", form_location: formId },
      })
    );

    if (typeof window !== "undefined" && (window as any).gtag) {
      (window as any).gtag("event", "generate_lead", { event_category: "TRT", event_label: "Form Submit", value: 1 });
    }
    if (typeof window !== "undefined" && (window as any).fbq) {
      (window as any).fbq("track", "Lead", { content_category: "TRT" });
    }

    const params = new URLSearchParams({ name, phone, location, source: "lp-trt-v2", service: "trt" });
    navigate(`/lp/trt-v2/thank-you?${params.toString()}`);
  };
  ...
  return (
    ...
    >
      <h3 className="text-[22px] font-bold mb-5 text-center" style={{ color: "#000033" }}>
        {heading}
      </h3>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="mb-3.5">
          ...
          <input
            ...
            onFocus={(e) => { e.currentTarget.style.borderColor = "#000033"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(0,0,51,0.08)"; }}
        ...
        <button
          type="submit"
          className="w-full inline-flex items-center justify-center gap-2 rounded-full px-8 py-3.5 text-[15px] font-bold cursor-pointer transition-all duration-200 border-none"
          style={{ background: "#E8670A", color: "#ffffff", letterSpacing: "0.03em", minHeight: 48 }}
        ...
        <div className="text-center mt-2 text-[11px]">
          <Link to="/privacy-policy" style={{ color: "#000033", textDecoration: "underline" }}>Privacy Policy</Link>
          {" · "}
          <Link to="/terms-of-service" style={{ color: "#000033", textDecoration: "underline" }}>Terms of Service</Link>
        </div>
      </form>
    </div>
  );
