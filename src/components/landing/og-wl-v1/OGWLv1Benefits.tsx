...
export const OGWLv1Benefits = () => (
  <section style={{ background: "#f8f9fa", padding: "clamp(48px, 8vw, 96px) 0" }}>
    <div className="max-w-[1200px] mx-auto px-6">
      <div className="text-center mb-12 md:mb-16">
        <h2 className="text-3xl md:text-4xl font-bold leading-tight" style={{ color: "#003366" }}>Why Us</h2>
        <p className="text-base mt-3 max-w-[600px] mx-auto" style={{ color: "#555555" }}>What's different about losing weight here.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cards.map((card) => (
          <div key={card.title} className="rounded-xl p-6" style={{ background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.06)" }}>
            <h3 className="font-bold text-base mb-2" style={{ color: "#003366" }}>{card.title}</h3>
            <p className="text-sm leading-relaxed" style={{ color: "#555555" }}>{card.desc}</p>
          </div>
        ))}
      </div>
    </div>
