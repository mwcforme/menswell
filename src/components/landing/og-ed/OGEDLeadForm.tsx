...
interface OGEDLeadFormProps {
  heading?: string;
  formId?: string;
}

export const OGEDLeadForm = ({
  heading = "Schedule My Private Visit",
  formId = "hero",
}: OGEDLeadFormProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.dispatchEvent(
      new CustomEvent("lp_ed_form_submit", {
        detail: { name, phone, location, source: "og-lp-ed", service: "ed", form_location: formId },
      })
    );

    if (typeof window !== "undefined" && (window as any).gtag) {
      (window as any).gtag("event", "generate_lead", { event_category: "ED", event_label: "Form Submit", value: 1 });
    }
    if (typeof window !== "undefined" && (window as any).fbq) {
      (window as any).fbq("track", "Lead", { content_category: "ED" });
    }

    const params = new URLSearchParams({ name, phone, location, source: "og-lp-ed", service: "ed" });
  ...
  return (
    ...
    >
      <h3 className="text-[22px] font-bold mb-5 text-center" style={{ color: "#003366" }}>
        {heading}
      </h3>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <label className="flex gap-2 items-start my-3.5 text-[11px] leading-relaxed cursor-pointer" style={{ color: "#6c757d" }}>
          <input
            ...
            style={{ accentColor: "#004883" }}
          />
          <span>I consent to receive SMS/text messages from Men's Wellness Centers. Message & data rates may apply. Reply STOP to opt out.</span>
        </label>
        {errors.consent && <p className="text-[12px] mb-2" style={{ color: "#dc3545" }}>{errors.consent}</p>}

        <button
          type="submit"
          className="w-full inline-flex items-center justify-center gap-2 rounded-lg px-8 py-3.5 text-[15px] font-bold cursor-pointer transition-all duration-200 border-none"
        ...
        <div className="text-center mt-3 text-[12px] flex flex-col gap-1 items-center" style={{ color: "#6c757d" }}>
          <span>Your privacy is our top priority. All visits are one-on-one.</span>
          <span className="inline-flex items-center gap-1">
            <Lock size={14} />
            HIPAA-compliant. Your info stays private.
          </span>
        </div>
