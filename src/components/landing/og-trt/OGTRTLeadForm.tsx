...
export const OGTRTLeadForm = ({ heading = "Get Started — At No Cost", formId = "hero" }: OGTRTLeadFormProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.dispatchEvent(new CustomEvent("lp_trt_form_submit", {
      detail: { name, phone, location, source: "og-lp-trt", service: "trt", form_location: formId },
    }));
    if (typeof window !== "undefined" && (window as any).gtag) (window as any).gtag("event", "generate_lead", { event_category: "TRT", event_label: "Form Submit", value: 1 });
    if (typeof window !== "undefined" && (window as any).fbq) (window as any).fbq("track", "Lead", { content_category: "TRT" });

    const params = new URLSearchParams({ name, phone, location, source: "og-lp-trt", service: "trt" });
