...
export const OGWLLeadForm = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.dispatchEvent(new CustomEvent("lp_weightloss_form_submit", {
      detail: { name, phone, location, source: "og-lp-weightloss-v2", service: "weight-loss", form_location: formId },
    }));

    if (typeof window !== "undefined" && (window as any).gtag) {
      (window as any).gtag("event", "generate_lead", { event_category: "Weight Loss", event_label: "Form Submit", value: 1 });
    }
    if (typeof window !== "undefined" && (window as any).fbq) {
      (window as any).fbq("track", "Lead", { content_category: "Weight Loss" });
    }

    const params = new URLSearchParams({ name, phone, location, source: "og-lp-weightloss-v2", service: "weight-loss" });
    ...
    >
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <label className="flex gap-2 items-start my-3.5 text-[11px] leading-relaxed cursor-pointer" style={{ color: "#6c757d" }}>
          ...
          <span>By checking this box, you agree to receive text messages from Men's Wellness Centers. Message & data rates may apply. Reply STOP to opt out.</span>
        </label>
