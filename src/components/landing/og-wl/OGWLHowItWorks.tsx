    ...
    <section id="how-it-works" style={{ background: "#003366", color: "white", padding: "clamp(48px, 8vw, 96px) 0" }}>
      <div className="max-w-[1200px] mx-auto px-6">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-3xl md:text-4xl font-bold leading-tight">Your Medical Weight Loss Plan in 3 Steps</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {steps.map((step, i) => (
            <div key={i} className="relative">
              ...
              {i < steps.length - 1 && (<div className="hidden md:block absolute top-1/2 -right-4 -translate-y-1/2 z-10" style={{ color: "rgba(255,255,255,0.3)" }}><ChevronRight size={24} /></div>)}
            </div>
          ))}
        </div>
