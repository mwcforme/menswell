...
export const OGTRT2HowItWorks = () => {
  const steps = [
    ...
    { number: "02", icon: TestTube, title: "Get Same-Day Lab Results", description: "Quick, in-clinic blood draw. No fasting required. Your physician reviews results with you on the spot." },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {steps.map((step, i) => (
            <div key={i} className="relative">
              <div className="text-center p-8 rounded-xl h-full flex flex-col" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-5 text-lg font-bold" style={{ background: "rgba(0,72,131,0.3)", border: "2px solid #4da6ff", color: "#4da6ff" }}>{step.number}</div>
                <div className="flex justify-center mb-4"><step.icon size={28} strokeWidth={1.5} style={{ color: "rgba(255,255,255,0.7)" }} /></div>
                <h3 className="text-xl font-semibold mb-3">{step.title}</h3>
                <p className="text-[15px] leading-relaxed flex-1" style={{ color: "rgba(255,255,255,0.7)" }}>{step.description}</p>
              </div>
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 -right-4 -translate-y-1/2 z-10" style={{ color: "rgba(255,255,255,0.3)" }}><ChevronRight size={24} /></div>
              )}
            </div>
          ))}
        </div>
