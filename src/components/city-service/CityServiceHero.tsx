...
export const CityServiceHero = ({ location, service }: Props) => {
  const scrollToForm = () => {
    document.getElementById("location-cta")?.scrollIntoView({ behavior: "smooth" });
  };

  return (
    <section id="hero" className="relative flex flex-col overflow-hidden" style={{ background: "#151933" }}>
      <div className="absolute inset-0">
        <img
          src={service.heroImage}
          alt={`${service.serviceName} in ${location.city}, Virginia`}
          className="w-full h-full object-cover"
        />
        <div
          className="absolute inset-0"
          style={{ background: "linear-gradient(to bottom, rgba(21,25,51,0.58) 0%, rgba(21,25,51,0.68) 100%)" }}
        />
      </div>
      ...
      >
        ...
        >
          {service.heroHeadline(location.city)}
        </h1>

        <p
          className="mx-auto mb-5 md:mb-6 text-base md:text-lg leading-relaxed"
          style={{ maxWidth: 700, color: "rgba(255,255,255,0.85)", textShadow: "0 1px 3px rgba(0,0,0,0.4)" }}
        >
          {service.heroSubtext(location.city, location.phone)}
        </p>
        ...
        >
          ...
          <span style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", fontWeight: 500 }}>
            Medically reviewed by {location.leadPhysician}
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
          <button
            ...
            style={{ background: "#F97316", color: "#FFFFFF", padding: "16px 32px", fontSize: 13, letterSpacing: "0.05em", height: 52 }}
          ...
          <a
            ...
            style={{ background: "#0F2A4A", color: "#FFFFFF", padding: "16px 32px", fontSize: 13, letterSpacing: "0.05em", height: 52, textDecoration: "none", border: "none" }}
          >
            <Phone size={15} strokeWidth={2.5} />
            Call {location.city}: {location.phone}
          </a>
        </div>
      </div>
